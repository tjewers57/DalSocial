...
const Profile = () => {
    ...
    const getStatus = async () => {
        try {
            ...
            const targetUser = await axios.get(`http://${process.env.BACKEND_API}:8080/users/getbyemail/` + email);
            if(currentUser.data == '' || targetUser.data == ''){
                alert("Error, invalid user.");
            }

            //this will grab the status of the relationship between the two users
            const blockStatus = await axios.get(`http://${process.env.BACKEND_API}:8080/block/status/` + targetUser.data.id + '/' + currentUser.data.id);
            if(blockStatus.data){
                setIsBlocked(true);
            }
        } catch (error) {
    ...
    const saveChanges = async (e) => {
        ...
        fetchUser().then(async u => {
            ...
            try {
                const profile = await axios.get(`http://${process.env.BACKEND_API}:8080/profiles/getbyuser/` + u.id);
                const data = {
                    id: profile.data.id,
                    title,
                    bio,

...
const FriendRequest = (userEmail) => {
    ...
    const getStatus = async () => {
        try {
            ...
            const targetUser = await axios.get(`${getBackendApi()}/users/getbyemail/` + userEmail.userEmail);
            if(currentUser.data === '' || targetUser.data === ''){
                alert("Error, invalid user.");
            }
            
            const requestStatus = await axios.get(`${getBackendApi()}/friend-requests/checkrequeststatus/` + currentUser.data.id + '/' + targetUser.data.id);
    ...
    const acceptRequest = async (e) => {
        ...
        try {
            ...
            const requestResponse = await axios.post(`${getBackendApi()}/friend-requests/acceptbyusers/` + targetUser.data.id + '/' + currentUser.data.id);
    ...
    const sendRequest = async (e) => {
        ...
        try {
            ...
            const requestResponse = await axios.post(`${getBackendApi()}/friend-requests/` + currentUser.data.id + "/send-friend-request/" + targetUser.data.id);
